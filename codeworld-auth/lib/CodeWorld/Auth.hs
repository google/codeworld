...
-- | Read authentication configuration from given application directory
...
-- | Textual representation of authentication method
authMethod :: AuthConfig -> String
authMethod (Google _) = "Google"
authMethod (Local _) = "Local"
authMethod None = "not configured"

-- | Wrapper around Snap handlers that require an authenticated client
...
-- | Wrapper around Snap handlers that can work with authenticated or
...
-- | Provide additional Snap routes specific to authentication method in effect

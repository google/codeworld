...
opts =
    info
        (helper <*> config)
        (fullDesc <> progDesc "CodeWorld gameserver stresstest client" <>
         header
  ...
  where
    ...
    config =
        ...
        option
            auto
            (long "events" <> short 'e' <> showDefault <> metavar "M" <>
             value 100 <>
...
main = do
    ...
    connect config $ \conn -> do
        ...
        results <-
            mapConcurrently id $
            waitForStart config conn :
            replicate (clients config - 1) (joinGame config gid)
        end <- getTime Monotonic
        let consistent = all (== head results) (tail results)
        if consistent
            then putStrLn "All clients got consistent data."
            else putStrLn "The clients got different results!"
        putStrLn $ "Events sent:         " ++ show (nrequests config)
        putStrLn $
            "Running time was:    " ++
            show (timeSpecToS (end - start) * 1000) ++ "ms"
        putStrLn $
            "Requests per second: " ++
            show (fromIntegral (nrequests config) / timeSpecToS (end - start))

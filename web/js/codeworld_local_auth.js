...
var LocalAuth = (() => {
  ...
  function onPreConfirmSignIn() {
    ...
    return new Promise(resolve => {
      httpPost({
          url: SIGN_IN_URL,
          headers: { "Authorization": `Basic ${btoa(formData.userId + ":" + formData.password)}` }
        })
        .then(resp => resolve({
        ...
        .catch(e => {
          if (isPasswordExpired(e)) {
            return sweetAlert2({
              ...
              html:
                `<input id="${USER_ID}" class="swal2-input" placeholder="Enter your user name" value="${Html.encode(formData.userId)}">` +
  ...
  function onPreConfirmReset() {
    ...
    return new Promise(resolve => {
      httpPost({
          url: SIGN_IN_URL,
          data: { newPassword: formData.newPassword1 },
          headers: { "Authorization": `Basic ${btoa(formData.userId + ":" + formData.password)}` }
        })
        .then(resp => resolve({
          userId: formData.userId,
